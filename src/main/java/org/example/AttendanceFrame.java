...
public class AttendanceFrame extends JFrame {
            ...
            if (dayCounter < maxDays) {
                ...
                for (int i = 0; i < resultGui2.model5.getRowCount(); i++) {
                    ...
                    if (idObj != null && idObj.toString().equals(id)) {
                        ...
                        idExists = true;
                        name = resultGui2.model.getValueAt(i, 0).toString();
                        pos = resultGui2.model.getValueAt(i, 1).toString();

                        System.out.println(name + " " + pos);
                        break;
                    }
                }
                JavaGui javaGuiFrame = new JavaGui("Payroll System",resultGui2,id,month,year,day,checkin,checkout,totalMonthlyMinutes,presentCount,absentCount,lateCount,name,pos);
                javaGuiFrame.namefield.setText(name);
                javaGuiFrame.posfield.setText(pos);
                if(idExists){
                    javaGuiFrame.namefield.setEditable(false);
                    javaGuiFrame.posfield.setEditable(false);
                }
    ...
    public void markAttendance(String status) {
        ...
        int test;
        try {
            test = Integer.parseInt(idField.getText().trim());
            if (test < 0 || test == -1) {
                JOptionPane.showMessageDialog(null, "Invalid ID. Please enter a valid positive number (not -1).", "Invalid ID", JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "ID must be a valid integer.", "Invalid Input", JOptionPane.ERROR_MESSAGE);
            return;
        }
